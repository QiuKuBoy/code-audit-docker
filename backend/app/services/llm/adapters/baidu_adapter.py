...
class BaiduAdapter(LLMAdapter):
    """Baidu Wenxin ERNIE adapter via Qianfan API"""
    ...
    async def chat(self, messages: list[dict], tools: Optional[list[dict]] = None) -> LLMResponse:
        # Baidu v2 API is OpenAI-compatible
        body: dict = {
            "model": self.model,
            "messages": [],
        }
        ...
        for msg in messages:
            ...
            if role == "system":
                ...
                body["messages"].append({
                    "role": "user",
                    "content": f"Tool result ({msg.get('tool_call_id', 'unknown')}): {msg['content']}"
                })
