"""ByteDance Doubao adapter (Volcano Engine Ark)"""
...
class DoubaoAdapter(LLMAdapter):
    """ByteDance Doubao adapter via Volcano Engine Ark API (OpenAI-compatible)"""
    ...
    async def chat(self, messages: list[dict], tools: Optional[list[dict]] = None) -> LLMResponse:
        # Ark API is OpenAI-compatible, use httpx directly to avoid openai SDK version issues
        body: dict = {
            "model": self.model,
            "messages": [],
        }
